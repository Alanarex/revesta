            ...
            <ul class="navbar-nav align-items-center">
                @auth
                    <li class="nav-item me-3">
                        ...
                        @endphp
                        <a class="nav-link text-dark position-relative" href="#" id="bookmarksDropdown"
                            ...
                            <div>
                                ...
                                @else
                                    @foreach ($recentBookmarks as $rb)
                                        @if ($rb->blog)
                                            <a href="{{ route('blogs.show', $rb->blog->slug ?? $rb->blog->id) }}"
                                                class="dropdown-item d-flex align-items-start">
                                                <div class="flex-grow-1">
                                                    <div class="fw-semibold">{{ Str::limit($rb->blog->title, 70) }}</div>
                                                    <div class="small text-muted">
                                                        {{ $rb->blog->user?->full_name ?? 'Auteur' }}</div>
                                                </div>
                                                <button
                                                    class="btn btn-sm btn-link text-danger bookmark-toggle-inline ms-3 mb-0"
                                                    type="button" data-blog-id="{{ $rb->blog->id }}">✕</button>
                                            </a>
                                        @endif
                                    @endforeach
                                @endif
