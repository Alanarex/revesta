...
=======
        $destroyRoute =
            $isAdmin && !$isViewingOwn ? route('admin.users.destroy', $user) : route('admin.users.destroy', $user);
...
=======
            ...
            <x-layout.card class="shadow-sm">
                ...
                <x-forms.form formId="updateInfoForm" action="{{ $updateRoute }}" method="PUT">
                    <x-inputs.text-input label="Prénom" name="first_name" :value="$user->first_name" icon="fa-user"
                        placeholder="Entrez votre prénom" :readonly="!$canEdit" />
                    <x-inputs.text-input label="Nom" name="last_name" :value="$user->last_name" icon="fa-user"
                        placeholder="Entrez votre nom" :readonly="!$canEdit" />
                    <x-inputs.email-input label="Email" name="email" :value="$user->email" icon="fa-envelope"
                        placeholder="exemple@email.com" :readonly="!$canEdit" />
                    <x-inputs.text-input label="Téléphone" name="phone" :value="$user->phone" icon="fa-phone"
                        placeholder="Votre numéro de téléphone" :readonly="!$canEdit" />
                    @if (auth()->user()->isAdmin() && !$isViewingOwn && !empty($rolesOptions))
                        <x-inputs.select-input label="Rôle" name="role_id" :value="$user->role_id" icon="fa-user-shield"
                            placeholder="Selectionner un role" :options="$rolesOptions" :required="true" :readonly="!$canEdit" />
                    @endif
                    <x-inputs.select-input label="Statut civil" name="civil_status" :value="$user->civil_status" icon="fa-heart"
                        :options="$civilStatuses" :readonly="!$canEdit" />
                    <x-inputs.select-input label="Statut familial" name="family_status" :value="$user->family_status" icon="fa-home"
                        :options="$familyStatuses" :readonly="!$canEdit" />
                    <x-inputs.textarea-input label="Biographie" name="bio" :value="$user->bio" icon="fa-pen"
                        rows="4" placeholder="Entrez votre biographie..." :readonly="!$canEdit" />
            ...
            @if ($canEdit && !($isAdmin && auth()->id() !== $user->id))
                <x-layout.card class="shadow-sm">
                    ...
                    <x-forms.form formId="changePasswordForm" action="{{ route('admin.users.update-password', $user) }}"
                        method="PUT">
                        <x-inputs.password-input label="Ancien mot de passe" name="current_password" icon="fa-key"
                            placeholder="Entrez votre mot de passe" required />
                        <x-inputs.password-input label="Nouveau mot de passe" name="password" icon="fa-lock"
                        ...
                        <x-inputs.password-input label="Confirmer le mot de passe" name="password_confirmation"
                            icon="fa-circle-check" placeholder="Confirmez le mot de passe" required />
            ...
            <div class="row g-4 mb-4">
                        ...
                        <x-slot name="header">
                            ...
                            @if (($simulations ?? collect())->count() > 3)
                                <div class="d-flex gap-2">
                                    <button class="btn btn-sm btn-outline-secondary simulations-prev" type="button">
                                        <i class="fas fa-chevron-left"></i>
                                    </button>
                                    <button class="btn btn-sm btn-outline-secondary simulations-next" type="button">
                    ...
                    @if ($canEdit && !($isAdmin && auth()->id() !== $user->id))
                        <x-layout.card class="shadow-sm">
                            ...
                            <x-forms.form formId="changePasswordForm"
                                action="{{ route('admin.users.update-password', $user) }}" method="PUT">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="mt-2">
                        ...
                        <div class="progress" style="height:6px;">
                            <div class="progress-bar" role="progressbar" style="width: {{ $profileScore ?? 0 }}%;"
                                aria-valuenow="{{ $profileScore ?? 0 }}" aria-valuemin="0" aria-valuemax="100">
                            </div>
                        </div>
                    </div>
                                ...
                                <x-slot name="header">
                                    ...
                                    @if ($isViewingOwn)
                                        <div class="mt-3 flex-wrap" id="blog-filters">
                                            <button type="button" class="blog-filter-btn is-active" data-tag="all"
                                                style="--filter-color: #0d6efd;">Tous</button>
                                            <button type="button" class="blog-filter-btn" data-tag="published"
                                                style="--filter-color: #198754;">Publie</button>
                                            <button type="button" class="blog-filter-btn" data-tag="draft"
                                                style="--filter-color: #6c757d;">Brouillon</button>
                                            <button type="button" class="blog-filter-btn" data-tag="pending"
                                                style="--filter-color: #ffc107;">En attente</button>
                                            <button type="button" class="blog-filter-btn" data-tag="rejected"
                                                style="--filter-color: #dc3545;">Rejete</button>
                                            <button type="button" class="blog-filter-btn" data-tag="bookmarked"
                                                style="--filter-color: #0dcaf0;">
