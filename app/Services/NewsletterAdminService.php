 ...
 * 
 * Provides admin utilities for managing newsletter subscribers,
 * including bulk operations, export, and analytics.
 * 
 * @phpstan-type NewsletterStats array{
 *   total: int,
 *   verified: int,
 *   unverified: int,
 *   recently_subscribed: int
 * }
 */
class NewsletterAdminService
{
    ...
    public function cleanupDuplicates(): int
    {
        ...
        foreach ($duplicates as $email) {
            ...
            for ($i = 1; $i < $subscribers->count(); $i++) {
                $subscribers[$i]->delete();
                $deleted++;
            }
        }
